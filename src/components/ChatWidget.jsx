...
export default function ChatWidget() {
  const initialState = [{ role: "assistant", content: "🕶️ **The Consigliere is here.** State your business." }];
    ...
    <div className="chat-widget border-2 border-amber-600 rounded-xl bg-zinc-900 flex flex-col h-[600px] w-full max-w-md mx-auto overflow-hidden shadow-2xl">
      <div className="p-4 bg-amber-600 flex justify-between items-center shrink-0">
        ...
        <div className="flex gap-4">
          <button onClick={downloadTranscript} className="text-black font-bold text-[10px] border border-black px-2 py-1 rounded">SAVE</button>
          <button onClick={() => window.confirm("Wipe?") && setMessages(initialState)} className="text-black font-bold text-[10px] border border-black px-2 py-1 rounded">WIPE</button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[90%] p-4 rounded-lg text-sm ${msg.role === "user" ? "bg-amber-600 text-black font-bold" : "bg-zinc-800 text-white border border-amber-900/30"}`}>
              <ReactMarkdown>{msg.content}</ReactMarkdown>
            </div>
          </div>
        ))}
        {loading && <div className="text-amber-500 animate-pulse text-[10px] font-bold">⌛ SECURING ENCRYPTION...</div>}
