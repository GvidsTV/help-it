...
export default function ChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    setMessages([...currentHistory, { role: "user", content: currentText }]);
    setInput("");
    setLoading(true);
    ...
    try {
      const res = await fetch("/.netlify/functions/chat", {
        ...
        body: JSON.stringify({ message: currentText, history: currentHistory.slice(-10) })
      });
  ...
  return (
    <div className="chat-widget shadow-2xl border border-amber-600/30 rounded-2xl bg-zinc-900 overflow-hidden flex flex-col h-[500px] w-full max-w-md mx-auto">
      <div className="p-4 bg-amber-600 flex justify-between items-center shadow-lg">
        ...
        <button onClick={() => window.confirm("Wipe the record?") && setMessages(initialState)} className="text-black hover:scale-110 transition-transform">
          <Trash2 size={20} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/40">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${msg.role === "user" ? "bg-amber-600 text-black font-bold shadow-lg" : "bg-zinc-800 text-amber-50 border border-amber-900/50"}`}>
              {msg.content}
            </div>
          </div>
        ))}
      ...
      <div className="p-4 border-t border-amber-900/30 bg-zinc-900 flex gap-2">
        ...
        <button className="bg-amber-600 p-2 rounded-lg text-black disabled:opacity-50" onClick={sendMessage} disabled={loading || !input.trim()}>
          <Send size={18} />
        </button>
      </div>
    </div>
